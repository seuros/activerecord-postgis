    ...
    module PostGIS
      class VariableInterpolationTest < ActiveSupport::TestCase
        ...
        test "ST_Distance with variable interpolation in WHERE clause" do
          ...
          # Check the SQL to see if values are interpolated (this is the bug we're documenting)
          sql = query.to_sql
          assert_match(/POINT\([\d\.]+\s[\d\.]+\)/, sql, "SQL should show interpolated values (this demonstrates the issue)")
        ...
        test "ST_Distance with ST_MakePoint for comparison" do
          ...
          sql = query.to_sql
          assert_match(/ST_MakePoint\([\d\.]+, [\d\.]+\)/, sql, "SQL should show interpolated values in ST_MakePoint")
        ...
        test "ST_Distance with ST_GeomFromText and concatenation" do
          ...
          sql = query.to_sql
          assert_match(/ST_GeomFromText\('POINT\([\d\.]+\s[\d\.]+\)'/, sql, "SQL should show interpolated values in ST_GeomFromText")
