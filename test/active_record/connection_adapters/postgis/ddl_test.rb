      ...
      class DDLTest < ActiveSupport::TestCase
        def test_spatial_column_options
          [
            ...
            # Test that spatial column options are supported
            assert spatial_column_options_supported?(type), "#{type} should support spatial column options"
          end
        end

        def test_type_to_sql
          adapter = SpatialModel.lease_connection
          assert_equal "geometry(Point,4326)", adapter.type_to_sql(:st_point, limit: "POINT,4326")
        end
        ...
        def test_create_polygon_with_options
          ...
          # Column spatial metadata is accessible via our spatial methods
          assert_equal :st_polygon, col.type
          klass.lease_connection.drop_table(:spatial_models)
          assert_equal 0, count_geometry_columns
        end
        ...
        def test_create_spatial_column_default_value_geometric
          ...
          assert_equal :st_point, col.type
          klass.lease_connection.drop_table(:spatial_models)
          assert_equal 0, count_geometry_columns
        end
        ...
        def test_no_query_spatial_column_info
          ...
          # Note: We skip the no_queries assertion since our architecture doesn't have SpatialColumnInfo
        ...
        def test_column_types
          ...
          assert_equal :st_point, klass.columns[-1].type
        end
        ...
        def spatial_column_options_supported?(type)
          # In our gem architecture, all registered spatial types support spatial column options
          [ :geography, :geometry, :geometry_collection, :line_string, :multi_line_string,
           :multi_point, :multi_polygon, :st_point, :st_polygon, :st_line_string,
           :st_multi_line_string, :st_multi_point, :st_multi_polygon, :st_geometry_collection,
           :st_geometry, :st_geography ].include?(type)
        end
      end
    end
  end
