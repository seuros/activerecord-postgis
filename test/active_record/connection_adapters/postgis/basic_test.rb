...
class BasicTest < ActiveSupport::TestCase
  ...
  def test_postgis_available
    ...
    assert connection.adapter_name.include?("PostgreSQL")
    expected_postgis_lib_version_value = connection.select_value("SELECT postgis_lib_version()")
    assert_match(/^[2-3]\./, expected_postgis_lib_version_value)
  end
